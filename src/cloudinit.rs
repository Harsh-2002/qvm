...
# --- GRUB tweaks (timeout + framebuffer console resolution) ---------------
#
# We poke /etc/default/grub once, then regenerate the grub config exactly
# once. Both changes are idempotent (re-running first-boot is harmless).
#
# - GRUB_TIMEOUT: how long the menu waits before booting the default entry.
#   `0` is the qvm default — instant boot, no flashing menu.
# - video=1920x1080@60: tells the kernel framebuffer console (fbcon) to
#   render at 1920x1080 instead of the legacy 720x400 text mode. VNC
#   clients see a much crisper console with no CSS upscaling needed.
...
if [ -f /etc/default/grub ]; then
    ...
    # Append `video=` to the kernel cmdline once. If something else has
    # already set a video= mode we leave it alone.
    if ! grep -q 'video=' /etc/default/grub; then
        if grep -q '^GRUB_CMDLINE_LINUX_DEFAULT=' /etc/default/grub; then
            sed -i 's|^GRUB_CMDLINE_LINUX_DEFAULT="\(.*\)"|GRUB_CMDLINE_LINUX_DEFAULT="\1 video=1920x1080@60"|' /etc/default/grub
        else
            echo 'GRUB_CMDLINE_LINUX_DEFAULT="video=1920x1080@60"' >> /etc/default/grub
        fi
    fi
